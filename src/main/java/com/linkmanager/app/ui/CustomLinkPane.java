...
/**
 * This class represents a list of links that can be sorted and added via drag
 * and drop (DnD). Each instance will contain a CustomLinkList.
 * 
 * @author Ian Gardea
 *
 */
public class CustomLinkPane extends JPanel {
	...
	/**
	 * This constructor will initialize the components to be added to the link
	 * content pane.
	 * 
	 */
	...
	/**
	 * This function will create the various components to be added to the tab's
	 * content pane.
	 * 
	 ...
	 */
	public void refresh() {
		// Clear the panel of all content, and rebuild it based off of what is currently
		// in the link list.
		removeAll();
		...
		// Create a new panel for the buttons to allow control of the button's size free
		// of layout manager.
		...
		// This will be the boolean representation of the click state for each check box
		// (checked = true, unchecked = false).
		final Boolean[] listSelection = new Boolean[linkList.getModel().getSize()];

		// Create a new table that will hold the check-boxes, and an adjacent list of
		// links.
		// Customize the table model by disabling the editing features, which
		// maintaining the check-box functionality.
		...
		// This definition only serves to override to cell rendering such that the first
		// column is always a check-box.
			...
			public void actionPerformed(ActionEvent event) {
				...
				for (int i = 0; i < listSelection.length; i++) {
					if ((boolean) tableModel.getValueAt(i, checkBoxRow)) {
						...
						// Create a new thread for each active check box item after all pending threads
						// are finished
									...
									SwingUtilities.invokeLater(new Runnable() {
										public void run() {
											JOptionPane.showMessageDialog(LinkManager.getFrame(),
													e.getLocalizedMessage(), "Unable to open file",
		...
		// Fill the list selection check-boxes. By default, this will always empty
		// selections on refresh.
		for (int i = 0; i < listSelection.length; i++) {
		...
		// Required to see components after they are dynamically created (such as via
		// DnD).
		revalidate();
		repaint();
	}
