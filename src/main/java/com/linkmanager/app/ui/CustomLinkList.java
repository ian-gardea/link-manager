...
/**
 * This class represents a list of links, and defines the respective actions
 * that can be performed. Each link list has a name, which will be displayed in
 * the tab its contents are placed in.
 * 
 * @author Ian Gardea
 *
 */
public class CustomLinkList extends JList<CustomLink> {
	 ...
	 * @param index - the integer position of the link to retrieve from the link
	 *              list.
	 */
	...
	/**
	 * Adds a link to the link list using a specific name, target, and link type as
	 * a String. This is essentially a wrapper that will create a CustomLink object.
	 * 
	...
	/**
	 * Adds a link to the link list using a specific name, target, and link type as
	 * an integer. This is essentially a wrapper that will create a CustomLink
	 * object.
	 * 
	 * @param name   - the name of the link to be added to the link list.
	 * @param target - the target of the link to be added to the link list.
	 ...
	 * 
	 * This function overrides the parent function's behavior to avoid any further,
	 * potentially unnecessary processing.
	 * 
	 * @param index - the integer position of the link to be removed from the link
	 *              list.
	 */
	@Override
	...
	public void swapElements(int start, int target) {
		try {
			...
			if (start >= 0 && start < listModel.getSize() && target >= 0 && target < listModel.getSize()) {
			...
			JOptionPane.showMessageDialog(LinkManager.getFrame(), e.getLocalizedMessage(), "Unable to perform swap.",
					JOptionPane.ERROR_MESSAGE);
		} catch (final Exception e) {
			JOptionPane.showMessageDialog(LinkManager.getFrame(), e.getLocalizedMessage(), "Unknown Error",
					JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	public void promptRenameLink(int index) {
		...
		if (this.getLinkAt(index).getType() != CustomLink.SEP) {
			...
			String input = (String) JOptionPane.showInputDialog(LinkManager.getFrame(),
					"Please enter a new name for this link.", "Enter Link Name", JOptionPane.PLAIN_MESSAGE, null, null,
					getLinkAt(index).getName());
	...
	/**
	 * This function will prompt the user to rename the link's target at the
	 * specified index. It will return the original target if the user does not
	 * enter a target.
	 * 
	 * @param index - the integer position of the link in the link list.
	 */
	public void promptRetargetLink(int index) {
		...
		if (this.getLinkAt(index).getType() != CustomLink.SEP) {
			...
			String input = (String) JOptionPane.showInputDialog(LinkManager.getFrame(),
					"Please enter a new target for this link.", "Enter Link Target", JOptionPane.PLAIN_MESSAGE, null,
					null, getLinkAt(index).getTarget());
			...
			// If the user hits OK, and does not enter a name, re-prompt.
				...
				this.getLinkAt(index).setType(CustomLink.determineType(this.getLinkAt(index).getTarget()));
			}
		}
	}

	/**
	 * This function will prompt the user for the name and target of a new link to
	...
	public void promptAddLink() {
		...
		int result = JOptionPane.showConfirmDialog(LinkManager.getFrame(), myPanel, "Enter Link Information",
		...
		if (result == JOptionPane.OK_OPTION) {
			if (nField.getText().equals("") || tField.getText().equals("")) {
				...
				CustomLink temp = new CustomLink(nField.getText(), tField.getText(),
						CustomLink.determineType(tField.getText()));
	...
	public void promptAddLink(String target) {
		String input = JOptionPane.showInputDialog(LinkManager.getFrame(),
				"Please enter the name for the link pointing to:\n\"" + target + "\"", "Enter Link Name",
	...
	/**
	 * This function will add a separator to the list of links. The separator is
	 * treated like a link to it can be rearranged or deleted, however it does not
	 * have a target.
	 * 
	 */
	public void addSeparator() {
		// Make the link and update the link list.
		CustomLink temp = new CustomLink("______________________________________________________________________");
		listModel.addElement(temp);
	}
